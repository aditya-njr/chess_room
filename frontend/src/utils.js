...
export function statusMessage(state) {
  ...
  if (status === 'draw_insufficient_material') return 'Draw — insufficient material.'
  if (status === 'draw_repetition') return 'Draw — threefold repetition.'
  if (status === 'draw_fifty_moves') return 'Draw — fifty-move rule.'
  if (status === 'resigned') return `${capitalize(winner)} wins by resignation.`
...
export const API_BASE = import.meta.env.DEV ? '/api' : 'http://127.0.0.1:8000'
...
export function wsUrl(roomCode) {
  ...
  return `ws://127.0.0.1:8000/ws/${roomCode}`
}
